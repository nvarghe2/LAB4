...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   ...
   { //start of the printArray method
   ...
   */
   ...
   public int findMinimum( int column )
   {
	   ...
	   for (int i = 0; i < intArray.length; i++) 
	   {
		   if (minV > intArray[i][column]) 
			   {
				   minV = intArray[i][column];
				}
		   animate(i, column, minV); 
	   }
	   System.out.println("Minimum Value in column " + column + " = " + minV);

	   return minV;// replace this line with your return statement
   ...
   */
   ...
   {	//start of countFound method
	   System.out.println("Count Frequency of value= " + value + " in 2D ");

	   int count = 0; 
	   for (int i= 0; i< intArray.length; i++) 
	   {
		   for (int j = 0; i < intArray[i].length; i++) 
		   {
	   ...
	   System.out.println("No. of times value = " + value + " occured is = " + count);
